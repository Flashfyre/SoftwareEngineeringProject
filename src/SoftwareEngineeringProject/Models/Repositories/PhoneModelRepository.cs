...
public class PhoneModelRepository : IRepository<PhoneModel, string>
{
    ...
    {
        return context.PhoneModels.Include(pm => pm.Manufacturer).Include(pm => pm.Phones).ThenInclude(p => p.VendorPhones).ThenInclude(vp => vp.Vendor).Include(pm => pm.Phones).ThenInclude(p => p.Carrier).AsEnumerable();
    }
    ...
    {
        return context.PhoneModels.Include(pm => pm.Manufacturer).Include(pm => pm.Phones).ThenInclude(p => p.VendorPhones).ThenInclude(vp => vp.Vendor).Include(pm => pm.Phones).ThenInclude(p => p.Carrier).SingleOrDefault(q => q.PhoneModelID == id);
    }
